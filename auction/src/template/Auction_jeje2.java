...
public class Auction_jeje2 implements AuctionBehavior {
	...
	private List<Double> saved_too_high_profits= new ArrayList<Double>();
	private List<Double> saved_ok_profits= new ArrayList<Double>();
	...
	public void auctionResult(Task previous, int winner, Long[] bids) {
		...
		if (winner == agent.id()) {
			...
			saved_ok_profits.add(expected_profit);
			...
			saved_too_high_profits.add(expected_profit);
			Double avg_too_high = saved_too_high_profits.stream().mapToDouble(Double::doubleValue).sum() / saved_too_high_profits.size();
			Double avg_ok =0.0;
			if(saved_ok_profits.size()>0) avg_ok= saved_ok_profits.stream().mapToDouble(Double::doubleValue).sum() / saved_ok_profits.size();
			...
			expected_profit = (avg_ok+avg_too_high)/ (saved_ok_profits.size()+saved_too_high_profits.size());
	...
	public List<Plan> plan(List<Vehicle> vehicles, TaskSet tasks) {
		System.out.println(saved_too_high_profits.stream().mapToDouble(Double::doubleValue).sum() / saved_too_high_profits.size());
		System.out.println(saved_ok_profits.stream().mapToDouble(Double::doubleValue).sum() / saved_ok_profits.size());
